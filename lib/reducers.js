...
function pendingApplications (state, action) {
  ...
  case actions.RECEIVED_FORM:
    var req = action.req
    var context = req.message.object.context
    var application = find(state, app => {
      return app.permalink === context
    })
    ...
    var link = req.payload.link
    var existing = utils.findFormState(application.forms, link)
  ...
  case actions.CREATE_VERIFICATION:
    var application = utils.getPendingApplication(state, action.context)
  ...
  case actions.APPROVE_PRODUCT:
    var application = utils.getPendingApplication(state.pendingApplications, action.context)
    var updatedProducts = (state.products[productType] || []).concat(application)
  ...
  case actions.APPROVED_PRODUCT:
    var product = utils.getPendingApplication(state.products, action.context)
    var updatedProduct = clone(product, { product: action.permalink })
    var updatedProducts = utils.replace(state.products[productType], product, updatedProduct)
    return clone(state, {
      products: clone(state.products, {
        [productType]: updatedProducts
      })
...
function updateWithVerification (pendingApplications, application, form, verification) {
  var verifications = [
    ...form.verifications,
    {
  ...
  return utils.replace(pendingApplications, application, updatedApp)
}
