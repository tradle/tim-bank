...
module.exports = exports = function reducer (state, action) {
  ...
  case actions.NEW_CUSTOMER:
    return reducers.newCustomerState(action.customerRootHash)
...
function forms (state, action) {
  ...
  case actions.RECEIVED_FORM:
    ...
    const existing = utils.findFormState(state, curHash)
    ...
    return state.concat({
      ...
      form: {
        ...
        body: req.parsed ? req.parsed.data : JSON.parse(req.data.toString()),
        txId: action.txId
      },
      verifications: []
    })
...
function verifications (state, action) {
  ...
  case actions.CREATE_VERIFICATION:
    ...
    var form = utils.findFormState(state.forms, verifiedItem[CUR_HASH])
  ...
  case actions.RECEIVED_VERIFICATION:
    ...
    var parts = verification.document.id.split('_')
    var type = parts[0]
    var hash = parts[1]

    var form = utils.findFormState(state.forms, hash)
    var existing = form.verifications && utils.find(form.verifications, v => v[CUR_HASH] === msg[CUR_HASH])
    if (existing) return state
  ...
  case actions.SENT_VERIFICATION:
    // mutation! bad
    const body = action.sentVerification.parsed.data
    extend(action.rawVerification.body, body)
    action.rawVerification[CUR_HASH] = action.sentVerification[CUR_HASH]
    return state
  }
}
...
function newVerificationFor (state, verifiedItem, identity) {
  const doc = verifiedItem.parsed.data
  ...
  verification.document = {
    id: doc[TYPE] + '_' + verifiedItem[CUR_HASH],
    title: doc.title || doc[TYPE]
  }

  verification.documentOwner = {
    id: types.IDENTITY + '_' + verifiedItem.from[ROOT_HASH],
    title: verifiedItem.from.identity.name()
  }
...
function products (state, action) {
  switch (action.type) {
  case actions.APPROVE_PRODUCT:
    const productType = action.product
    const acquiredProduct = {
      type: productType
    }

    const updatedProducts = (state.products[productType] || []).concat(acquiredProduct)
  ...
  case actions.APPROVED_PRODUCT:
    // mutation! bad
    const product = action.product
    product.rootHash = action.rootHash
    return state
  }
}
