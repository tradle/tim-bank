...
module.exports = exports = function reducer (state, action) {
  ...
  case actions.CREATE_VERIFICATION:
    ...
    var pending = utils.getApplication(state, action.application)
    var product = utils.getProduct(state, action.application)
    var application = pending || product
    var form = utils.findFormStateLenient(application.forms, verifiedItem)
    var verification = newVerificationFor(state, form, identityInfo.object)
    if (pending) {
      var pendingApplications = updateWithVerification(state.pendingApplications, application, form, {
        object: verification,
        author: identityInfo
      }, true)
      ...
      var applications = state.products[productType]
      var updated = updateWithVerification(applications, application, form, {
        object: verification,
        author: identityInfo
      })
  ...
  case actions.ASSIGN_RELATIONSHIP_MANAGER:
    return clone(state, {
      relationshipManager: action.employee.permalink
    })
...
function pendingApplications (state, action) {
  ...
  case actions.RECEIVED_VERIFICATION:
    ...
    var verifiedItemInfo = utils.parseObjectId(verification.object.document.id)
    var form = utils.findFormStateLenient(application.forms, verifiedItemInfo)
    if (hasVerification(form, verification.link)) return state
...
function verifications (state, action) {
  ...
  case actions.SENT_VERIFICATION:
    ...
    action.rawVerification.permalink = action.sentVerification.permalink
    return state
  }
}
...
function newVerificationFor (customerState, formState, identity) {
  ...
  const doc = formInfo.body
  let verification = utils.getImportedVerification(customerState, doc) || {}
  ...
  verification.documentOwner = {
    id: types.IDENTITY + '_' + customerState.permalink,
  ...
  if (verifications && verifications.length) {
    verification.sources = verifications.map(v => v.body)
  }
...
function updateWithVerification (applications, application, form, verification, issued) {
  var newVerification = {
    link: verification.link,
    permalink: verification.permalink,
    txId: verification.txId,
    body: verification.object,
    author: tradleUtils.pick(verification.author, 'permalink', 'link')
  }

  var vProp = issued ? 'issuedVerifications' : 'verifications'
  var existing = form[vProp] || []
  var verifications = [
    ...existing,
    newVerification
  ]
...
function hasVerification (form, vlink) {
  const all = form.verifications.concat(form.issuedVerifications || [])
  return find(all, v => v.link === vlink)
}
