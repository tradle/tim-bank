...
Bank.prototype.receiveMsg = function (msgBuf, senderInfo) {
  var self = this;
  return this.tim.receiveMsg(msgBuf, senderInfo).then(function (entry) {
...
Bank.prototype._onMessage = function (msg) {
  ...
  var res = {};
  var from = req.from;
  return this._getCustomerState(from).catch(function (err) {
...
Bank.prototype._handleDocument = function (req) {
  ...
  return this._send(req, verification).then(function (entries) {
...
Bank.prototype._setResource = function (type, rootHash, val) {
  ...
  return Q.ninvoke(this._db, 'put', prefixKey(type, rootHash), val).then(function () {
...
Bank.prototype.send = function (req, resp, opts) {
  ...
  return maybeSign.then(function (signed) {
...
Bank.prototype.destroy = function () {
  ...
  this._destroyPromise = Q.all([this.tim.destroy(), Q.ninvoke(this._db, 'close')]);
...
function dumpDBs(tim) {
  ...
  identities.onLive(function () {
    identities.createValueStream().on('data', function (result) {
  ...
  messages.onLive(function () {
    messages.createValueStream().on('data', function (data) {
      tim.lookupObject(data).then(function (msg) {
