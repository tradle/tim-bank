...
function simpleBank(opts) {
  ...
  bank.use(types.GET_MESSAGE, lookupAndSend.bind(bank));
  bank.use(types.GET_HISTORY, sendHistory.bind(bank));
...
function handleDocument(req, res) {
  ...
  return bank.send(req, verification).then(function (entries) {
...
function sendNextFormOrApprove(req) {
  ...
  if (next) {
    ...
    resp = utils.buildSimpleMsg('Please fill out this form and attach the snapshot of the original document', next);
...
function lookupAndSend(req) {
  ...
  return bank.tim.lookupObject(info).then(function (obj) {
...
function sendHistory(req) {
  ...
  return bank.tim.history(from).then(function (objs) {
...
function getRelevantPending(pending, reqState) {
  var found;
  var docType = reqState[TYPE] === types.VERIFICATION ? getType(reqState.parsed.data.document) : reqState[TYPE];
