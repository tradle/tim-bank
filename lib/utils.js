...
//         return !v[SIGNEE] || v[SIGNEE].indexOf(verifierHash) === -1
  ...
  findFormStateLenient: function (forms, query) {
    if (typeof query === 'string') {
      return this.findFormState(forms, { permalink: query })
    }

    const { link, permalink } = query
    if (link && permalink) {
      return this.findFormState(forms, { link }) || this.findFormState(forms, { permalink })
    }

    return this.findFormState(forms, query)
  },
