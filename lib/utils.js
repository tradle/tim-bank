...
var utils = module.exports = {
  ...
  getUnverifiedForms: function getUnverifiedForms (state, productModel) {
    return utils.getForms(productModel)
      ...
      .map(f => {
        return {
          [TYPE]: f,
          [CUR_HASH]: state.forms[f].form[CUR_HASH]
        }
      })
  },
  ...
  validateResource: function (resource, model) {
    ...
    required = required.filter(name => name !== 'from' && name !== 'to')
    ...
    if (missing.indexOf('photos') === -1) {
      msg = 'Oops! You left a required field blank. Please edit.'
    } else {
      if (resource.subClassOf === 'tradle.Form') {
    ...
    // const fieldMessages = {}
    // missing.forEach(name => fieldMessages[name] = 'This field is required')
    return {
      message: msg,
