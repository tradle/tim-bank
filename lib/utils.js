...
var BUILTIN_MODELS = mergeModels()
  .add(baseModels)
  .add(customModels)
  .get()
...
var utils = module.exports = {
  ...
  getUnverifiedForms: co(function* ({
    bank,
    verifier,
    application,
    forms
  }) {
    if (!verifier) verifier = bank.node.identity
    if (!forms) forms = yield bank.getRequiredForms({ application })
    ...
    forms.forEach(f => {
      ...
      const fState = utils.findLast(application.forms, form => form.type === f)
...
//         return !v[SIGNEE] || v[SIGNEE].indexOf(verifierHash) === -1
  ...
  getApplication: function (apps, appPermalinkOrContext) {
    return appPermalinkOrContext && utils.find(apps.pendingApplications || apps, app => {
      return app.context === appPermalinkOrContext ||
        app.permalink === appPermalinkOrContext
    })
  },
  ...
  isVerifiableForm: function (model) {
    return model.verifiable !== false && model.subClassOf === 'tradle.Form'
  },
  ...
  getReferencedModels: function ({ models, subset }) {
    const refs = getReferences({ models, subset })
    return refs.filter(ref => !(ref in BUILTIN_MODELS))
  },

  getMyProductType: function (productType) {
