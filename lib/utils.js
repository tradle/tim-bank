...
var utils = module.exports = {
  ...
  getUnverifiedForms: function getUnverifiedForms (verifierHash, state, productModel) {
    return utils.getForms(productModel)
      .filter(f => {
        const docState = state.forms[f]
        if (!docState || !docState.verifications) return true

        return docState.verifications.every(v => {
